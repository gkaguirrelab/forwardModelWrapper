function results = WrapperAnalyzePRFLocalAnalysis(stimFileName,flywheelApi,downloadPath,subjectID,dataFileType,tr,outpath,varargin)
% Wrapper to manage inputs to Kendrick Kay's analyze pRF code
%
% Syntax:
%  WrapperAnalyzePRF(stimFileName,flywheelApi,downloadPath,subjectID,dataFileType,tr,outpath)
%
% Description:
...
%                           stimulus.
%   flywheelApi           - String. Flywheel API
%   downloadPath          - String. Path to download icafix results
%   sunjectID             - String. Id of the subject you want to analyze
...
%   'daraFileName'         - String. Use this to point to single run.
%
% Outputs:
%   none
%
...
p.addRequired('flywheelApi',@isstr);
p.addRequired('downloadPath',@isstr);
p.addRequired('subjectID',@isstr);
...
p.addParameter('dataFileName', @isstr)

% parse
p.parse(stimFileName, flywheelApi, downloadPath, subjectID, dataFileType, tr, outpath, varargin{:})

%% Load Data

scratchSaveDir = icafixData(downloadPath, flywheelApi, subjectID);
if scratchSaveDir(end) == '/'
    directory = strcat(scratchSaveDir, '*/*/MNINonLinear/Results');
else
    directory = strcat(scratchSaveDir, '/*/*/MNINonLinear/Results');
end    
...
if scratchSaveDir(end-1:end) ~= "gz"
    
    d = dir(directory); % Find the acquisitions
...
if scratchSaveDir(end-1:end) ~= "gz" % This part does it for multiple runs
     for ii = 1:runNumber
